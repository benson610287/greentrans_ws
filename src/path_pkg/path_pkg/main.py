...
class MinimalSubscriber(Node):
    ...
    def __init__(self):
        ...
        self.srv = self.create_service(Allinfo, '/path_plan', self.task_callback)
    ...
    def task_callback(self, request, response):
        ...
        self.create_mission()
        self.creat_robot(len(data["robot"]))
        ...
        response.path=str(self.tmp)
        response.message="done"
        return response
    # def change_map_json_to_CSR(self,map_json):
    #     num_nodes = len(map_json)
    ...
    #         cur_offset += len(map_json[node]["edges"])
            
    #         edges = edges + map_json[node]["edges"]
    #         weights = weights + map_json[node]["weights"]
    ...
    def creat_robot(self,robot_num: int):
        self.robot_data = {
            ...
            "carrying_capacity":[10] * robot_num
        }
    ...
    def creat_data_model(self):
        ...
        n_vehicles = len(self.robot_data)

        # a pickup order and a delivery order are distinct with additional pad for the depot with 0 demand
        n_orders = len(self.transport_order_data) * 2
        n_orders = len(self.robot_data) * 2
        ...
        self.data_model.set_vehicle_locations(cudf.Series(self.start_node),cudf.Series(self.end_node))
        for i in range(n_orders):
            self.data_model.add_vehicle_order_match(i+1,cudf.Series([i]))
        #===============================================================
        ...
        # print(order_demand)
        #================================================================
        pickup_order_locations = cudf.Series([self.target_map[loc] for loc in self.transport_order_data['pickup_location'].to_arrow().to_pylist()])
        delivery_order_locations = cudf.Series([self.target_map[loc] for loc in self.transport_order_data['delivery_location'].to_arrow().to_pylist()])
        ...
        # self.data_model.set_min_vehicles(2)
        #================================================================
        ...
        self.data_model.set_pickup_delivery_pairs(pickup_orders, delivery_orders)
        #================================================================
    ...
    def compute_path(self):
        ...
        for i in waypoint_route.values_host:
            ...
            if i[1]=="Delivery":
                # print("aa")
                break
        self.get_logger().info(f'route={self.tmp}')
